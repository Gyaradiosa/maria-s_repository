...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/17 23:21:39 by maduarte          #+#    #+#             */
/*   Updated: 2024/03/17 23:21:45 by maduarte         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int main(int argc, char *argv[]) {
    ...
    if (digits == 1) {
        ...
        while (multiplier > 0) {
            int digit = number / multiplier;
            if (digit != 0) {
                char *temp_result = convert_to_written_form(digit * multiplier, dict_path);
                if (composite_result == NULL) {
                    composite_result = my_strdup(temp_result);
                } else {
                    char *temp = malloc(strlen(composite_result) + strlen(temp_result) + 2);
                    strcpy(temp, composite_result);
                    strcat(temp, " ");
                    strcat(temp, temp_result);
                    free(composite_result);
                    composite_result = temp;
                }
                free(temp_result);
            }
            number %= multiplier;
